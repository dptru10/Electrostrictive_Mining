...
for item in centrosymm: 
	cen_data.append(mpr.query(criteria={"has":{"$all":["diel","elasticity"]},"band_gap":{"$gt":0.5},"spacegroup.point_group":{"$all":[item]},"e_above_hull":{"$lt":0.2},"nelements":{"$gt":1}},properties=["diel"]))
	i+=1
i=0
print()
print()
...
for item in non_centro: 
	non_cen_data.append(mpr.query(criteria={"has":{"$all":["diel","elasticity"]},"band_gap":{"$gt":0.5},"spacegroup.point_group":{"$all":[item]},"e_above_hull":{"$lt":0.2},"nelements":{"$gt":1}},properties=["diel"]))
	j+=1
i=0
print()
print()
...
np.save('centro_diel_tensor.npy',cen_structures)
np.save('non_centro_diel_tensor.npy',non_cen_structures)
