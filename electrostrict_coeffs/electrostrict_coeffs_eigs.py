...
for i in range(len(dP)): 
	chi   = 1/eig_p[i]
	chi_ave_p.append((1/3*eps_0) * (chi))
	eps_ave_p.append((1/3*eps_0) * (eig_p[i]))#data_p['e11'].iloc[i] + data_p['e22'].iloc[i] + data_p['e33'].iloc[i])) 
...
for i in range(len(dP)): 
	chi   = 1/eig_m[i]
	chi_ave_m.append((1/3*eps_0) * (chi))
	eps_ave_m.append((1/3*eps_0) * (eig_m[i]))#data_m['e11'].iloc[i] + data_m['e22'].iloc[i] + data_m['e33'].iloc[i])) 
